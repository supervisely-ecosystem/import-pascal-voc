...
def import_pascal_voc(api: sly.Api, task_id, context, state, app_logger):
    if state["mode"] == "public":
        if state["trainval"] or state["train"] or state["val"]:
            trainval_archive = os.path.join(
                g.storage_dir, "VOCtrainval_11-May-2012.tar"
            )
            pascal_downloader(
        ...
        if state["test"]:
            ...
            pascal_downloader(
                g.pascal_test_dl_link, test_archive, "VOC2012test.tar", app_logger
            )
    else:
        remote_dir = state["customDataPath"]
        local_archive = os.path.join(
            g.storage_dir, os.path.basename(os.path.normpath(remote_dir))
        )
        file_size = api.file.get_info_by_path(g.team_id, remote_dir).sizeb
        if not file_exists(
            os.path.join(g.storage_dir, os.path.basename(os.path.normpath(remote_dir)))
        ):
        ...
        if "VOC2012" not in os.listdir(os.path.join(g.storage_dir, "VOCdevkit")):
            os.rename(
                os.path.join(g.storage_dir, "VOCdevkit", "VOC"),
